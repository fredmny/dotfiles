...
BRM_REPOS_DIR="$HOME/brm_repos"
DOTFILES_DIR="$HOME/personal/dotfiles/.config"
...
create_deployments_session() {
  ...
  tmux new-session -s "$session" -n 'k9s' -d
  tmux send-keys -t "$session:0" 'loga && k9s' C-m

  tmux new-window -t "$session:1" -n 'docker'
  tmux send-keys -t "$session:1" 'lazydocker' C-m
...
create_base_repo_session "datalake-pipelines" "$BRM_REPOS_DIR/datalake-pipelines-workdir/datalake-pipelines"
create_base_repo_session "infrastructure-live" "$BRM_REPOS_DIR/infrastructure-live"
create_base_repo_session "artisan" "$BRM_REPOS_DIR/artisan"
